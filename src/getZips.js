...
const loadCountyFIPSFromUGC = async (ugcCode) => {
  ...
  const fileStream = fs.createReadStream("../files/national_county.txt");
...
const loadZIPsFromFIPS = (fipsList) => {
  const workbook = xlsx.readFile("../files/zip_county.xlsx");
...
async function getLatestNwsAlerts() {
  ...
  try {
    ...
    console.log("NWS UNIQUE ZIPS FOUND", allUniqueZips);
